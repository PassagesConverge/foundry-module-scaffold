...
4. **Update src/shared.ts**
   - Change `MODULE_ID` constant to match your module.json `id`

5. **Update package-release.ps1**
   - Update the GitHub username in URLs (replace `yourusername`)

6. **Start developing**
   - Add your module logic to `src/main.ts`
   - Add custom styles to `styles/styles.css`
   - Run `npm run watch` while developing, or `npm run build` for a minified production bundle
   - Import additional TypeScript files from `src/main.ts`; esbuild bundles them into `dist/main.js`
...
├── src/
│   ├── main.ts          # TypeScript entry point with hooks
│   ├── shared.ts        # Shared utilities and MODULE_ID
│   ├── settings.ts      # Module settings registration
│   └── foundry.d.ts     # Minimal Foundry globals used by the scaffold
├── dist/
│   └── main.js          # Generated, minified production bundle
├── styles/
...
- **MODULE_ID must match everywhere**: module.json `id`, shared.ts constant, folder name
- **Do not edit `dist/main.js` directly**: run `npm run build` to regenerate it from `src/`
